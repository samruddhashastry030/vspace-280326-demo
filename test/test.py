...
class PresentModel:
    """
    Software golden model for PRESENT-80 (ISO/IEC 29192-2).
    Matches the serialised RTL exactly so test vectors are self-contained.
    """
    ...
    def load_key(self, key80: int):
        self.key = key80 & ((1 << 80) - 1)
    ...
    def encrypt(self):
        ...
        for rnd in range(1, 32):
            ...
            ns = 0
            for n in range(16):
                ns |= self.SBOX[(state >> (n*4)) & 0xF] << (n*4)
            state = ns
            # P-Layer (skip on round 31)
            if rnd < 31:
                ns = 0
                for i in range(64):
                    ns |= ((state >> i) & 1) << self.PLAYER[i]
                state = ns
            # Key schedule
            key = self._key_schedule(key, rnd)
        # Final whitening key XOR
        rk     = (key >> 16) & ((1 << 64) - 1)
        state ^= rk
        self.ct   = state
        self.done = True
...
# Official PRESENT-80 test vectors (Bogdanov et al., 2007)
TEST_VECTORS = [
...
# ── Self-check: verify model against official vectors at import time ──────────
_model_check = PresentModel()
for _pt, _key, _expected in TEST_VECTORS:
    _model_check.load_pt(_pt)
    _model_check.load_key(_key)
    _got = _model_check.encrypt()
...
async def load_key(dut, key80: int):
    """Shift 80-bit key in 10 bytes MSB-first with key_load = uio_in[0]."""
    for i in range(10):
...
async def load_pt(dut, pt64: int):
    """Shift 64-bit plaintext in 8 bytes MSB-first with pt_load = uio_in[1]."""
    for i in range(8):
...
async def start_encrypt(dut):
    """Pulse start for one clock on uio_in[2]."""
...
async def wait_done(dut, timeout=800):
    """Poll uio_out[0] (done flag) up to timeout cycles."""
...
async def read_ct(dut) -> int:
    """Read 64-bit ciphertext byte-by-byte via uo_out (byte_sel = uio_in[6:4])."""
    ct = 0
    for b in range(8):
        dut.uio_in.value = (b & 0x7) << 4
        await RisingEdge(dut.clk)
        ct |= int(dut.uo_out.value) << (b * 8)
    dut.uio_in.value = 0x00
    return ct
...
async def test_reset_behaviour(dut):
    """PWM and done must be low while rst_n=0."""
...
async def test_present_golden_vectors(dut):
    """Verify PRESENT-80 against all four official test vectors."""
...
async def test_music_pwm_output(dut):
    """After encryption, PWM on uo_out[0] must toggle (music engine active)."""
    ...
    model.encrypt()
    note  = model.get_note()
    dur   = model.get_duration()
    chord = model.is_chord()
    dut._log.info(f"Note index={note}, Duration nibble={dur}, Chord={chord}")
...
async def test_pause_resume(dut):
    """
    With ui_in[0]=0 (pause), PWM must freeze.
    Setting ui_in[0]=1 (resume) must re-enable note advancement.
    """
    ...
    snapshot = int(dut.uo_out.value) & 0x01
    await ClockCycles(dut.clk, 30)
    assert (int(dut.uo_out.value) & 0x01) == snapshot, \
        "PWM changed while paused — note advanced unexpectedly!"
...
async def test_avalanche_effect(dut):
    """Two keys differing by 1 bit must produce >=16-bit different ciphertexts."""
    ...
    dut._log.info(
        f"KEY_A -> 0x{ct_a:016X}  KEY_B -> 0x{ct_b:016X}  bit-diff={diff_bits}"
    )
